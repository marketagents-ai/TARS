...
from tars_bot.api_client import call_api
from tars_bot.repo_processor import count_files, start_background_processing, repo_processing_event
...
from tars_bot.repo_processor import start_background_processing, repo_processing_event, count_files

def setup_bot(args, gh_conn, inverted_index_search, api):
    repo = gh_conn.repo
    ...
    async def generate_prompt_command(ctx, file_path, *, user_task_description):
        ...
        try:
            repo_code = gh_conn.get_file_content(file_path)
            ...
            principles_markdown = gh_conn.get_local_file_content('principles.md')
            ...
            if principles_markdown.startswith("Error reading local file:"):
                ...
                principles_markdown = gh_conn.get_file_content('principles.md')
            ...
            principles = gh_conn.extract_principles(principles_markdown)
            ...
            prompt = gh_conn.generate_prompt(file_path, repo_code, principles, user_task_description, code_type)
    ...
    async def dir_command(ctx, max_depth: int = 0):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                structure = await bot.loop.run_in_executor(executor, gh_conn.get_dir, repo, "", "", max_depth)
